...
## 2024-05-18 - Improved Press Animations
**Learning:** Adding subtle scaling interactions to touchable components adds a highly tactile, polished feel while confirming registering touches, vastly improving perceived responsiveness without altering design layouts.
**Action:** Use a combination of `remember { MutableInteractionSource() }`, `.collectIsPressedAsState()`, `animateFloatAsState(spring(stiffness = Spring.StiffnessMediumLow))`, and `Modifier.graphicsLayer { scaleX = scale; scaleY = scale }` together with `.clickable` for standard button responses on generic panel components.
