...
## 2025-03-03 - Added missing roles and onClickLabel for generic clickable containers
**Learning:** Generic clickables in layout views are not properly communicated by TalkBack.
**Action:** Add `role = Role.Button` and `onClickLabel` to `Modifier.clickable()` where they are absent on generic containers.
