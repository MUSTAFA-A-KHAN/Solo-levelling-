...
## 2025-03-03 - Tactile Feedback and Accessibility for Welcome Buttons
**Learning:** Generic buttons built using custom containers (`SystemPanel`) with a simple `.clickable` feel static and lack semantic meaning for screen readers. Overriding the default ripple with a spring-based scale animation and explicit roles creates a much more modern, responsive feel.
**Action:** When building interactive UI elements in Compose, enhance the `.clickable` modifier with a `MutableInteractionSource` to drive `animateFloatAsState(spring())` scaling feedback, while simultaneously supplying `role = Role.Button` and `onClickLabel` for clear screen reader context.
