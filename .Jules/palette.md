...
## 2025-03-03 - Compose Custom Buttons Accessibility (Role & onClickLabel)
**Learning:** When making generic containers like `Box` or `Row` interactive via the `.clickable` modifier, they lack context for TalkBack screen readers and are announced as unlabelled elements.
**Action:** Enhance `.clickable` modifiers by explicitly passing `onClickLabel` and `role = Role.Button` to improve semantic meaning for accessibility tools.
